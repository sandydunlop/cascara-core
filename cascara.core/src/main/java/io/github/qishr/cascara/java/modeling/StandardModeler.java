...
public class StandardModeler implements Modeler<Module, Package, Class<?>, Field, Method, Parameter> {
    ...
    public TypeNode modelType(Class<?> type) {
        if (type.isEnum()) {
            return modelType(type, SemanticNode.Kind.ENUM);
        } else if (type.isInterface()) {
            return modelType(type, SemanticNode.Kind.INTERFACE);
        } else if (type.isRecord()) {
            return modelType(type, SemanticNode.Kind.RECORD);
        } else if (type.isAnnotation()) {
            return modelType(type, SemanticNode.Kind.ANNOTATION);
        } else {
            return modelType(type, SemanticNode.Kind.CLASS);
        }
    }

    @Override
    public ClassNode modelClass(Class<?> type) {
        return (ClassNode)modelType(type, SemanticNode.Kind.CLASS);
    }
    ...
    private TypeNode modelType(Class<?> type, SemanticNode.Kind kind) {
        TypeNode typeNode;
